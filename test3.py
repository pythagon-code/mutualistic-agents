...
class Actor(nn.Module):
    def __init__(self, vec_dim: int, embed_dim: int) -> None:
        ...
        self._cos_sim = CosineSimilarity(dim = -1)
        self._enc_a = FNN(
            input_size = vec_dim,
            hidden_size = 16,
            num_hidden_layers = 4,
    ...
    def train(self, critic: "Critic", vec_a: Tensor, vec_b: Tensor, target: Tensor) -> None:
        ...
        critic_loss_head = critic.forward(
            vec_a = vec_a,
            vec_b = vec_b,
...
class Critic(nn.Module):
    ...
    def eval_mse(self, actor: Actor, batch_size: int = 4096) -> Tensor:
        with torch.no_grad():
            vec_a = torch.rand(batch_size, self._vec_dim, device = device)
            vec_b = torch.rand(batch_size, self._vec_dim, device = device)
            embed_a = actor._enc_a_target(vec_a)
            embed_b = actor._enc_b_target(vec_b)
            actor_head_in = torch.cat([embed_a, embed_b], dim = -1)
            pred = actor._head_target(actor_head_in).squeeze(-1)
            target = actor._cos_sim(vec_a ** 3, vec_b ** 3)
            target_loss = (pred - target) ** 2
            state_action_a = torch.cat([vec_a, embed_a], dim = -1)
            state_action_b = torch.cat([vec_b, embed_b], dim = -1)
            critic_embed_a = self._enc_a_target(state_action_a)
            critic_embed_b = self._enc_b_target(state_action_b)
            head_in = torch.cat([critic_embed_a, critic_embed_b, pred.unsqueeze(-1)], dim = -1)
            pred_loss = self._head_target(head_in).squeeze(-1)
            return mse_loss(pred_loss, target_loss)
...
def test_mutual_cosine_similarity() -> None:
    ...
    for i in trange(20000, desc = "mutual cosine"):
        ...
        if i % 250 == 0:
            actor_mse = actor.eval_mse()
            critic_mse = critic.eval_mse(actor = actor)
            print(f"step: {i}, actor_mse: {actor_mse.item()}, critic_mse: {critic_mse.item()}")

    final_mse = actor.eval_mse(batch_size = 4096)
    assert final_mse < 5e-3
